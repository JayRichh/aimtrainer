...
export function WeaponSystem({
  ...
  const applyRecoil = useCallback(() => {
    ...
    recoilVelocityRef.current.x = THREE.MathUtils.clamp(recoilVelocityRef.current.x, -RECOIL_MAX_ANGLE, RECOIL_MAX_ANGLE)
    recoilVelocityRef.current.y = THREE.MathUtils.clamp(recoilVelocityRef.current.y, 0, RECOIL_MAX_ANGLE)
  }, [currentWeapon])
  ...
  const shootLaserGun = () => {
    ...
    const laserMaterial = new THREE.LineBasicMaterial({ color: 0xff0000, linewidth: 3 })
  ...
  useFrame((_, delta) => {
    ...
    bulletsRef.current.forEach((bullet, bulletIndex) => {
      ...
      if (bullet instanceof THREE.Mesh) {
        ...
        // Special handling for explosive or timed weapons
        if (
  ...
  const explode = (bullet: THREE.Mesh) => {
    ...
    targets.forEach(target => {
      ...
      if (distance < explosionRadius) {
        const hitScore = Math.max(100 - (distance / explosionRadius) * 100, 10)
        onHit(target.id, hitScore)
      }
    })
  ...
  return (
    <group ref={weaponRef}>
      <Weapon
        ...
        onShoot={shoot}
        muzzleFlashRef={muzzleFlashRef} // Pass the muzzleFlashRef to Weapon
      />
    </group>
  )
}
